...
**MVP(현재): 텍스트 패키지** — 제목·대본·썸네일 글자·설명·태그·챕터·수익화 코칭

**2차: 썸네일 이미지 자동 생성**
- [ ] `/api/thumbnail` 신설 — `thumbnailPrompt` → 이미지 생성 API(나노바나나 등) 래핑
- [ ] 시니어 친화 썸네일 템플릿(큰 글자 오버레이) 합성
...
영상 자동 편집/렌더링, TTS 음성합성은 MVP 밖. "올리기 직전까지의 준비물"에 집중하고,
실제 영상 조립은 사용자가 휴대폰 기본 앱이나 무료 툴로 진행하도록 안내합니다.
